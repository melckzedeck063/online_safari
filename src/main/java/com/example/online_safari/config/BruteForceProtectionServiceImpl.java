...
@Service
public class BruteForceProtectionServiceImpl implements com.example.online_safari.config.BruteForceProtectionService {
    ...
    private final ConcurrentHashMap< String, com.example.online_safari.config.FailedLogin> cache;
    ...
    protected com.example.online_safari.config.FailedLogin getFailedLogin(final String username){
        com.example.online_safari.config.FailedLogin failedLogin = cache.get(username.toLowerCase());

        if(failedLogin ==null){
            //setup the initial data
            failedLogin = new com.example.online_safari.config.FailedLogin(0, LocalDateTime.now());
